...
//  if ( (retval = readSerialPort(device, &readInt, readStr)) > 0 ){
...
int TankLevel::readSerialPort(char *device){
  ...
  char readChar = 0;
  fprintf(stderr, "dbg: readChar is initialised to: %d\n", readChar);

  int readInt = 0;
  fprintf(stderr, "dbg: readInt is initialised to: %d\n", readInt);
  ...
  while (readChar != 10){
	...
	fprintf(stderr, "dbg: read() success, readChar=%c\n", readChar);
      }
	...
	readInt =  readChar;
	fprintf(stderr, "dbg: write() success, readInt=%d\n", readInt);
  ...
  fprintf(stderr, "dbg: write() success, charNum=%d, readStr=%s\n",
	  charNum, readStr);
