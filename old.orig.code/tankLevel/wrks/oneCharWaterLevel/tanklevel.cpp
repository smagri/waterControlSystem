...
void TankLevel::updatePercentageLevel(char *readStr){
  ...
  ui->sliderPercentageLevel->setValue(readInt * 100);
...
int TankLevel::readSerialPort(char *device, char *readStr){
  ...
  // readChar converted to an integer type for use in:
  ...
  while (readChar != 10){
	...
	fprintf(stderr,
		"dbg: readSP: write() success, readInt=%d\n", readInt);
      }
  ...
  readStr[1] = '\0'; // string terminator = NULL = 0base10
  fprintf(stderr, "dbg: readSP: write() success, readStr=%s\n", readStr);
