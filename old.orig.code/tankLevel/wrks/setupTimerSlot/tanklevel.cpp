...
TankLevel::TankLevel(QWidget *parent) :
  ...
  //connect( &timer, SIGNAL(timeout()), this, SLOT(setupTimerSlot()) );
  ...
  // Character read from serial port converted to a string.  
  // 
  // Space on heap  via new has to be  done here(class definition) not
  // in   .hpp(class   declaration).    However,   non-string(eg   int,
  // double...) can be defined in .hpp???
  //
  ...
  // 	   this, SLOT(readSerialPort(device, readStr)) );
  // timer.start(1000); // this is a 1sec timer

  // Testing timer->timoutSignal slot/callback fn.
  setupTimerSlot();
...
void TankLevel::setupTimerSlot(){

//cout<<"dbg: setupTimerSlot: in timer->TimoutSignal callback function."<<endl; 
  fprintf(stderr,
...
//	if ( (readChar != 10) && (readChar != 13) && (numDigitsRead > 9) ){
  ...
  // ignore first xxx digits
  // if ( numDigitsRead < (3*10) )
  //   return 10;
  // else
  //   numDigitsRead = 1;
  ...
  readStr[numDigitsRead] = '\0'; // string terminator = NULL = 0base10
