...
void TankLevel::updatePlotLevel(){
  ...
  ui->qwtPlot->setAxisTitle(QwtPlot::yLeft, "Hight  (kilo litres)");
...
void TankLevel::updateAbsoluteLevel(){
  ...
  ui->sliderAbsoluteLevel->setValue(levelValue);
  levelValueStr = QString::number(levelValue);
  ui->labelAbsLevel->setText(levelValueStr);
...
void TankLevel::updatePercentageLevel(){
  ...
  int levelValue = 55;
  //char levelValueStr[sizeof(levelValue)+1] = "";
  QString levelValueStr;
  ...
  //ui->sliderPercentageLevel->setScale(1, 100, TRUE);
  ui->sliderPercentageLevel->setScale(1, 100, 5); // min,max,step
  ui->sliderPercentageLevel->setScaleMaxMinor(5); // max num of minor ticks


  ui->sliderPercentageLevel->setValue(levelValue);
  levelValueStr = QString::number(levelValue);
  ui->labelPercentLevel->setText(levelValueStr);
  // Or manually, needs to be made to work for 3digit numbers(vs 2 digits here)
  // levelValueStr[0] = ((levelValue / 10) + '0');
  // levelValueStr[1] = ((levelValue % 10) + '0');
